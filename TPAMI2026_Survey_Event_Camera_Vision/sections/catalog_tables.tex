...
\begin{table*}[p]
\centering
\caption{Dedicated 2D reconstruction paradigms. The primary division is the sensing input: event-only methods infer visual appearance from events, whereas event-guided methods restore or synthesize RGB frames using complementary event measurements. Diffusion denotes a task-trained diffusion model unless the method appears in the Foundation-Prior table.}
...
\rowalt
TimeReplayer~\cite{he2022timereplayer} & CVPR'22 & \iconev\iconcam & Video & CNN & Frame interpolation \\
...
\rowalt
EGDeblurring~\cite{xie2025egdeblurring} & CVPR'25 & \iconev\iconcam & Sharp image & Diff. & Deblurring \\
ERetinex~\cite{guo2025eretinex} & ICRA'25 & \iconev\iconcam & Image & CNN & Low-light \\
\rowalt
...
Fourier prior~\cite{she2025fourier} & ACM MM'25 & \iconev\iconcam & Image & CNN & Low-light \\
\rowalt
MamEVSR~\cite{xiao2025mamevsr} & CVPR'25 & \iconev\iconcam & HR video & SSM & Super-resolution \\
\rowalt
...
E2E-3D~\cite{xu2025neuromorphic3d} & ICME'25 & \iconev & Mesh & Learned geometry & Object reconstruction \\
\rowalt Elite-EvGS~\cite{zhang2024eliteevgs} & ICRA'25 & \iconev & Novel views & 3DGS & E2V-task prior \\
...
EvHDR-NeRF~\cite{chen2025evhdrnerf} & AAAI'25 & \iconev\iconcam & HDR views & NeRF & Single exposure \\
\rowalt EvaGaussians~\cite{yu2025evagaussians} & ICCV'25 & \iconev\iconcam & Novel views & 3DGS & Blurry RGB \\
...
EvDiff~\cite{li2025evdiff} & arXiv'25 & 2D & \iconev / E2V & Large-scale image-trained latent prior & Surrogate training and distillation \\
...
Task-trained diffusion without a large-scale pretrained model (TRG-Diffusion, EGDeblurring, NEC-Diff) remains in \cref{tab:recon_catalog}; E2V teachers (Elite-EvGS and EventSplat) are task-pretrained priors rather than foundation models. Event-to-event super-resolution is treated as sensor-stream restoration and is outside these visual reconstruction tables.
\end{tcolorbox}
\end{table*}
