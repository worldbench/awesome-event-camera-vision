...
\noindent\circnum{2}~\textbf{Diffusion-Based Paradigm Shift.} The introduction of diffusion models~\cite{ho2020ddpm} has fundamentally changed generative event reconstruction, beginning in 2024. E2VIDiff~\cite{liang2024e2vidiff} first applied diffusion priors to event-to-video reconstruction, demonstrating superior perceptual quality compared to discriminative baselines. TRG-Diffusion~\cite{zhu2024trgdiffusion} incorporated temporal residual guidance to maintain temporal consistency. HDRev-Diff~\cite{yang2025hdrevdiff} extended diffusion priors to HDR reconstruction. EVDiffuser~\cite{chen2025evdiffuser} introduced a modality-adaptive diffusion pipeline for consistent video enhancement.
...
Early work by Paredes-Vall{\'e}s and de Croon~\cite{paredes2021backtoeventbasics} demonstrated that photometric constancy --- the principle that a reconstructed image, when used to re-simulate events, should produce events consistent with the input --- provides a powerful self-supervised signal. EvINR~\cite{wang2024evinr} revisited event generation models through implicit neural representations for self-supervised event-to-video reconstruction.
...
Events provide temporal information that can guide spatial super-resolution of low-resolution frames. STIR~\cite{lu2023stir} learned spatial-temporal implicit neural representations for event-guided video SR. eSL-Net++~\cite{yu2023eslnetplus} jointly addressed blur and low resolution. EvTexture~\cite{kai2024evtexture} demonstrated event-driven texture enhancement at ICML'24. MamEVSR~\cite{xiao2025mamevsr} employed state space models, and EvEnhancer~\cite{lu2025evenhancer} (CVPR'25 Highlight) achieved continuous space-time super-resolution with strong generalizability.


\paragraph{HDR Reconstruction.}
The inherent high dynamic range of event cameras enables HDR imaging from events alone or in combination with conventional LDR frames. NeurImg-HDR~\cite{han2023neurimghdr} fused neuromorphic and conventional images for hybrid HDR. HDRev-Net~\cite{yang2023hdrevnet} learned event-guided HDR video reconstruction. EventHDR~\cite{zou2025eventhdr} extended this to high-speed HDR video generation. SEE~\cite{lu2025see} introduced a broader light range enhancement approach at NeurIPS'25, expanding beyond traditional HDR to handle extreme illumination variations.


\paragraph{Low-Light Enhancement.}
Events maintain high signal quality in low-light conditions due to their sensitivity to relative brightness changes. EvLowLight~\cite{liang2023evlowlight} proposed coherent event-guided low-light video enhancement. NER-Net~\cite{liu2024rled} addressed nighttime perception. EvLight~\cite{liang2024evlight} contributed a large-scale real-world dataset and approach for event-guided low-light enhancement, later extended to EvLight++~\cite{chen2026evlightpp}. ERetinex~\cite{guo2025eretinex} connected event cameras with classical Retinex theory.
...
\noindent\circnum{1}~\textbf{Foundational Event NeRFs.} Three concurrent works established this direction in 2023. Ev-NeRF~\cite{hwang2023evnerf} proposed an event-based NeRF that learns from accumulated event counts. EventNeRF~\cite{rudnev2023eventnerf} demonstrated that a NeRF can be learned from a single color event camera, without requiring any conventional frames. E-NeRF~\cite{klenk2023enerf} formulated a physically grounded event generation model within the NeRF framework, enabling training from a moving event camera.

\noindent\circnum{2}~\textbf{Event-Enhanced NeRF from Degraded Images.} A major application of events in NeRF is to overcome image degradation. E2NeRF~\cite{qi2023e2nerf} used events to enhance NeRF training from motion-blurred images. BeNeRF~\cite{li2024benerf} reconstructed a NeRF from a single blurry image and its accompanying event stream. E3NeRF~\cite{qi2024e3nerf} improved efficiency for event-enhanced reconstruction. EvDeblurNeRF~\cite{cannici2024evdeblurnerf} and Deblur-e-NeRF~\cite{low2024deblurenerf} specifically addressed motion blur under high-speed or low-light conditions. EBAD-NeRF~\cite{qi2024ebadnerf} introduced event-driven bundle adjustment for joint pose refinement and deblurring.

\noindent\circnum{3}~\textbf{Robust and Specialized NeRFs.} Robust-e-NeRF~\cite{weng2023robustenerf} handled sparse and noisy events under non-uniform motion. AE-NeRF~\cite{feng2025aenerf} augmented event NeRFs for non-ideal conditions and larger scenes. EvHDR-NeRF~\cite{chen2025evhdrnerf} leveraged events' HDR capability to build high dynamic range radiance fields from single-exposure images. LSE-NeRF~\cite{liao2025lsenerf} learned sensor modeling errors for deblurred NeRFs using RGB-event stereo. SaENeRF~\cite{zhang2025saenerf} focused on suppressing artifacts specific to event-based NeRF rendering.

\noindent\circnum{4}~\textbf{Dynamic Scene NeRFs.} DE-NeRF~\cite{ma2023denerf} introduced deformable neural radiance fields using both RGB and event cameras for dynamic scenes. EvDNeRF~\cite{bhattacharya2024evdnerf} reconstructed event data itself with dynamic NeRFs. DynEventNeRF~\cite{rudnev2025dyneventnerf} reconstructed general dynamic scenes from multi-view RGB and event streams.
...
\noindent\circnum{2}~\textbf{Deblurring via Event-Guided 3DGS.} A major application mirrors the NeRF deblurring paradigm: using events to recover sharp 3D representations from blurry inputs. EaDeblur-GS~\cite{weng2024eadeblurgs} introduced event-assisted 3D deblur reconstruction. BeSplat~\cite{matta2025besplat} reconstructed Gaussian Splats from a single blurry image and event stream. DiET-GS~\cite{lee2025dietgs} combined diffusion priors with event streams for motion deblurring 3DGS --- notably bridging generative reconstruction (diffusion) with 3D representation (3DGS). EBAD-GS~\cite{deng2025ebadgaussian} extended event-driven bundle adjustment to Gaussian Splatting. DeblurSplat~\cite{zhang2025deblursplat} achieved SfM-free deblurring 3DGS using DUSt3R for initialization and events for supervision.

\noindent\circnum{3}~\textbf{Event-Enhanced 3DGS.} EvaGaussians~\cite{yu2025evagaussians} used event streams to assist Gaussian Splatting from blurry images. E2GS~\cite{deguchi2024e2gs} enhanced 3DGS with event data for improved quality. EF-3DGS~\cite{liao2025ef3dgs} enabled free-trajectory reconstruction. E-3DGS~\cite{yin2025e3dgs} incorporated both exposure and motion events.
...
\noindent\circnum{5}~\textbf{Dynamic and 4D Reconstruction.} EventBoosted-3DGS~\cite{xu2025eventboosted3dgs} introduced event-boosted deformable Gaussians for dynamic scene reconstruction. Ev4DGS~\cite{liu2025ev4dgs} addressed novel-view rendering of non-rigid objects from monocular event streams. E-4DGS~\cite{wang2025e4dgs} achieved high-fidelity dynamic reconstruction from multi-view event cameras with event-based initialization and event-adaptive slicing. DEGS~\cite{he2026degs} used event motion priors to guide deformation field optimization.
...
\node[methodbox=red!70!black] (e2vidiff) at (7.8, -1.5) {E2VIDiff};
...
\draw[arr=red!70!black] (diff) -- (e2vidiff);
...
Despite the availability of simulators, a significant domain gap persists between synthetic and real events due to noise characteristics, contrast threshold variations, and temporal dynamics that are difficult to model accurately. Bridging this sim-to-real gap is essential for methods trained on simulated data to generalize to real-world deployment. E2VID+~\cite{stoffregen2020hqf} addressed this gap by training on more realistic simulated data and introducing a principled photometric loss. Sim2Real-EVFI~\cite{zhang2024sim2realevfi} bridged the sim-to-real gap for event-based frame interpolation through per-scene optimization. EventFly~\cite{kong2025eventfly} (CVPR'25) introduced cross-platform adaptation strategies including EventBlend data mixing, enabling transfer across different event camera platforms and mounting configurations. More broadly, the self-supervised and cross-modal transfer methods discussed in \cref{sec:understanding_ov} provide indirect sim-to-real bridges by learning representations that are robust to domain shifts.
